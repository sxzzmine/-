...
class SURF_frameKeyP:
    ...
    def retainBest(self, num,frame):
        ...
        self.keypoints,self.descriptors = surf.compute(frame, self.keypoints)
    ...
    def computeKey(self, frame):
        self.keypoints = surf.detect(frame, None)

    def computeDescriptors(self, frame):
        # 计算关键点的描述符
        self.keypoints, self.descriptors = surf.detectAndCompute(frame, None)
...
class SURF_fileKeyP:
    def __init__(self, filenum,num_elements):
        ...
        self.frames = [SURF_frameKeyP(i) for i in range(num_elements)]
...
class SURF_Allfile:
    def __init__(self,num_files):
        ...
        self.files = [SURF_fileKeyP(i, 0) for i in range(num_files)]
